...
from . models import User, Customer, ProductList, AddedToCart, Orders, Logo, Review
...
def about(request):
    all_reviews = Review.objects.filter().all()
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            review = Review.objects.create(customer=request.user.customer, text=text, rating=int(rating))
            review.save()
            messages.info(request, 'Thank you for your honest review appreciate it')
            return redirect('about')
    else:
        messages.info(request, 'Please log in to leave a review. Thank you for your understanding.')
        return render (request,'about/about.html')

    return render (request,'about/about.html',{'all_reviews' : all_reviews})
...
def tables(request):
    products = ProductList.objects.filter(stall_owner=request.user).all()

    return render(request,'admin/tables.html', {'products': products})  
...
def update_product(request):
    if request.method == 'POST':
        ...
        products = ProductList.objects.filter(stall_owner=request.user, product_id=product_id)
        for product in products:
            ...
            product.save()
            messages.info(request, f"{products.count()} product(s) updated.")
...
def product_delete(request, product_id):
    ...
    product.delete()
    return render(request,'admin/tables.html', {'product': product})
...
def graphs(request):
    # Count the number of customers per delivery address for the current stall owner
    current_user = request.user
    stall_owner = ProductList.objects.filter(stall_owner=request.user)
    data = Customer.objects.values('delivery_address').annotate(count=Count('id')).order_by('delivery_address')
    labels = [d['delivery_address'] for d in data]
